...
export default function Home({curLocation, setCurLocation}) {
  ...
  return (
    <section class={`bg-cover bg-center h-[100vh] w-[100%] absolute`}  style={{ backgroundImage: `url(${HomeScreen})` }}>
        <Header></Header>
        <Search setPathCurIdx={setPathCurIdx} curLocation={curLocation} setCurLocation={setCurLocation}></Search>
        <dialog id="pathModal" className="modal"><PathPage curIdx={pathCurIdx} setCurIdx={setPathCurIdx} curLocation={curLocation}></PathPage></dialog>
    </section>
  )
}
